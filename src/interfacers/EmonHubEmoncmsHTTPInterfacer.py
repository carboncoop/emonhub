...
class EmonHubEmoncmsHTTPInterfacer(EmonHubInterfacer):
    ...
    def _process_post(self, databuffer):
        ...
        if not 'devicekey' in self._settings.keys() or str.__len__(str(self._settings['devicekey'])) != 32 \
                or str.lower(str(self._settings['devicekey'])) == 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx':
            devicekey=False

        if (not devicekey) and (not apikey):
            # Return true to clear buffer if the apikey is not set
            return True

        if apikey and devicekey:
            return True
        ...
        if apikey:
            post_url = post_url + 'apikey=' + self._settings['apikey']
        elif devicekey:
            post_url = post_url + 'devicekey=' + self._settings['devicekey']
    ...
    def set(self, **kwargs):
        ...
        for key, setting in self._cms_settings.iteritems():
            ...
            if key in self._settings and self._settings[key] == setting:
                ...
                if str.lower(setting[:4]) == 'xxxx':
                    self._log.warning("Setting " + self.name + " devicekey: obscured")
                    pass
                elif str.__len__(setting) == 32 :
                    self._log.info("Setting " + self.name + " devicekey: set")
                    pass
                elif setting == "":
                    self._log.info("Setting " + self.name + " devicekey: null")
                    pass
                else:
                    self._log.warning("Setting " + self.name + " devicekey: invalid format")
                    continue
                self._settings[key] = setting
                # Next line will log devicekey if uncommented (privacy ?)
                #self._log.debug(self.name + " apikey: " + str(setting))
                continue
